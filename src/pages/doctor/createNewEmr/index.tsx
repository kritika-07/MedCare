...
const CreateNewEmr = () => {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:3001/patients?status=Dalam antrian&dokter=${auth.name}`,
      {
        method: "GET",
      }
    )
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const users = await fetch(
        `http://localhost:3001/users?name=${values.pasien}`,
        {
          method: "GET",
        }
      );
      ...
      if (values.obat.length > 0) {
        ...
        const patients = await fetch(
          `http://localhost:3001/patients?nama=${values.pasien}&tanggal=${values.examinationDate}`,
          {
            method: "GET",
          }
        );
        ...
        const medicine = await fetch(
          `http://localhost:3001/medicine?nama=${values.obat[0]}`,
          {
            method: "GET",
          }
        );
